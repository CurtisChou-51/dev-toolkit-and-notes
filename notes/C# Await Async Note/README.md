...
## Task.Run 的使用誤區 (CPU Bound vs I/O Bound)

剛接觸非同步時會誤以為將耗時的方法包裝成 `Task.Run()` 就能提升效能或變成「非同步」，但實際上 `Task.Run()` 的作用只是將工作轉交給 ThreadPool 中的另一個背景執行緒去執行。CPU 密集工作中，無論換到哪個執行緒，總體都是要佔用一條執行緒進行運算。

### 使用時機
- I/O 密集型工作（打 API、查資料庫、讀寫檔案）：使用該方法自帶的非同步版本並搭配 `await` 來釋放執行緒。
- CPU 密集型工作（大量影像處理、複雜演算法）：直接在當前執行緒計算，或者考慮使用 `Parallel`、`PLINQ`。
